...
namespace {
    static const size_t PARALLEL_SORT_THRESHOLD = 1000;
    void parallel_sort_kmerseed(KmerSeedBucket::iterator begin, KmerSeedBucket::iterator end)
    {
        ...
        else
        {
            ...
            {
                parallel_sort_kmerseed(begin, mid);
            }
            #pragma omp task shared(mid, end)
            {
                parallel_sort_kmerseed(mid, end);
            }
            #pragma omp taskwait
...
std::unique_ptr<KmerList> count_kmer(const DnaBuffer& myreads)
{
    ...
    #pragma omp parallel
    {
        ...
        {
            parallel_sort_kmerseed(kmerseeds->begin(), kmerseeds->end());
        }
    }
